---
title: 产品图标参考
description: 按 ID 列出所有内置产品图标的完整参考。
---

# 产品图标参考

Visual Studio Code 包含一组内置图标，用于视图和编辑器中，也可用于悬停提示、状态栏以及扩展中。这些图标是**产品图标**（product icons），区别于贯穿 UI 中用于文件名旁边的**文件图标**（file icons）。

VS Code 内置的产品图标包含在 [Codicon 图标字体](https://github.com/microsoft/vscode-codicons) 中，构成**默认**的产品图标主题。扩展可以提供新的[产品图标主题](/vscode/extension/extension-guides/product-icon-theme)来重新定义这些图标，赋予 VS Code 全新的外观。

为此，所有产品图标都通过 ID 进行标识。图标标识符用于 UI 组件的标签中（`$(pencil)`）、API 中的 `ThemeIcon` 以及需要图标的贡献点中。

图标标识符与实际图标字体字形之间的关联由产品图标主题完成。

## 标签中的图标

图标可用于悬停提示中的 Markdown 标签、[StatusBarItem](/vscode/extension/references/vscode-api#StatusBarItem) 文本和 [QuickPickItem](/vscode/extension/references/vscode-api#QuickPickItem) 标签 API。在 Markdown 中添加图标的语法是 `$(iconIdentifier)`：
...
图标贡献点允许扩展通过 ID 定义额外的图标及其默认图标。之后，扩展（或依赖该扩展的其他扩展）可以在标签中（`$(iconId)`）或任何可以使用 `ThemeIcon` 的地方（`new ThemeIcon("iconId")`）使用该图标 ID。
...
产品图标主题可以重新定义图标（如果它们知道该图标 ID）。

## 图标列表

以下按标识符列出了所有内置产品图标。

图标 ID 标识了图标的使用位置。默认 codicon ID 描述了默认使用的 codicon 库中的图标，预览展示了该图标的外观。

[产品图标主题](/vscode/extension/extension-guides/product-icon-theme)可以单独替换每个图标，也可以替换 codicon 库中的所有图标。
